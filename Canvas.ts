  ...
  applyEvents(events: ShapeEvent[]): this {
    events.forEach((e) => {
        ...
        if (e.type === ShapeEventType.ShapeAdded) {
            ...
            const eShapeSelected = e as ShapeSelected;
            ...
            const eShapeUnselected = e as ShapeUnselected;
            ...
            const eReordered = e as ShapesReorderedToFront;
            ...
            const eReordered = e as ShapesReorderedToBack;
            this.reorderShapes(eReordered.shapesOrder);
            ...
            const eDeleted = e as ShapesDeleted;
...
// ----------------------------
  deleteSelectedShapes(multiArray, redraw: boolean) {
    // Collect IDs of shapes to be deleted
    const shapeIdsToDelete: number[] = [];
...
private deleteShapesByIds(shapeIds: number[], redraw: boolean): void {
  let updatedMultiSelectedArray = this.multiSelectedArray.map(subArray =>
      subArray.filter(shape => !shapeIds.includes(shape.id))
  );
...
// ----------------------------
  ...
  markNextInArray(selectedArray: Shape[], shapes: { [p: number]: Shape }) {
    ...
    // Control held without Alt
      ...
      if (selectedArray.length !== 0) {
        ...
        if (index !== -1) {
          ...
          this.shapeSelect(
            this.multiSelectedArray[index][selectedArray.length - 1].id,
            this.shapes
          );
        } else {
          ...
          this.shapeSelect(
            selectedArray[selectedArray.length - 1].id,
            this.shapes
          );
        }
      }
...
private reorderShapes(newOrder: number[]): void {
  this.shapesOrder = newOrder.map(id => this.getShapeById(id)).filter(shape => shape !== undefined) as Shape[];
}

public getShapesOrder(): number[] {
    return this.shapesOrder.map(shape => shape.id);
}

public getShapeById(id: number): Shape | undefined {
